...
describe('sqlite-v0 runtime search helpers', () => {
  ...
  test('search helper tokenizes text into prefix fts terms', () => {
    expect(__testing.toFtsMatchText('A "quoted" term')).toBe('a* OR quoted* OR term*');
  });

  test('search helper quotes punctuation-only text for fts', () => {
    expect(__testing.toFtsMatchText('"..."')).toBe('"""..."""');
  });
});
