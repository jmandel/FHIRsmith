...
describe('ValueSet $expand - Real-World Patterns', () => {
  ...
  describe('POST with Parameters Resource', () => {
    ...
    test('IR resolves registered supplement for tx-resource base system from unversioned request canonical', async () => {
      const seed = `${Date.now()}-${Math.floor(Math.random() * 100000)}`;
    ...
    test('IR fails fast when unversioned requested supplement is ambiguous across registered versions', async () => {
      const seed = `${Date.now()}-${Math.floor(Math.random() * 100000)}`;
    ...
    test('IR projects registered supplement extensions through generic overlay merge', async () => {
      const seed = `${Date.now()}-${Math.floor(Math.random() * 100000)}`;
    ...
    test('IR supplement-backed property filter drives total and pagination before output shaping', async () => {
      const seed = `${Date.now()}-${Math.floor(Math.random() * 100000)}`;
    ...
    test('IR text filter matches supplement designation values', async () => {
      const seed = `${Date.now()}-${Math.floor(Math.random() * 100000)}`;
