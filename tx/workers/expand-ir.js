...
class ExpandIRWorker extends ExpandWorker {
  ...
  async performExpansion(valueSet, params, logExtraOutput) {
    ...
    const externalLimit = this.externalLimit || EXTERNAL_DEFAULT_LIMIT;
    if (params.limit < -1) {
      params.limit = -1;
    } else if (params.limit > externalLimit) {
      params.limit = externalLimit;
    }
    ...
    const irResult = await maybeExpandValueSetViaIR({
      ...
      externalDefaultLimit: externalLimit,
